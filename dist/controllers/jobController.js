...
exports.createReview = exports.uploadJobPhoto = exports.updateJobStatus = exports.submitQuote = exports.getPlumberJobs = exports.createJob = void 0;
const client_1 = require("@prisma/client");
const database_1 = __importDefault(require("../utils/database"));
const createJob = async (req, res) => {
    try {
        const { title, description, jobType, urgency, address, latitude, longitude } = req.body;
...
const getPlumberJobs = async (req, res) => {
    try {
        const userId = req.user.id;
...
exports.getPlumberJobs = getPlumberJobs;
const submitQuote = async (req, res) => {
    try {
        ...
        if (job.status !== client_1.JobStatus.REQUESTED) {
        ...
        await database_1.default.job.update({
            where: { id: jobId },
            data: { status: client_1.JobStatus.QUOTED }
        });
        res.status(201).json({
...
const updateJobStatus = async (req, res) => {
    try {
        const { id: jobId } = req.params;
        const { status, scheduledAt, completedAt } = req.body;
        const userId = req.user.id;
        // Validate status
        if (!Object.values(client_1.JobStatus).includes(status)) {
        ...
        if (status === client_1.JobStatus.IN_PROGRESS && req.user.role === client_1.UserRole.PLUMBER) {
...
const uploadJobPhoto = async (req, res) => {
    try {
        ...
        // Create photo record
        const photo = await database_1.default.photo.create({
            data: {
                jobId,
                uploadedById: userId,
...
exports.uploadJobPhoto = uploadJobPhoto;
const createReview = async (req, res) => {
    try {
        ...
        if (job.status !== client_1.JobStatus.COMPLETED) {
